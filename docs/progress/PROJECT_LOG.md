...
- 创建 `KbDocumentChunkSearchResultVO`，明确返回切片 ID、所属文档 ID、切片序号和切片正文。
- 在 `DocumentChunkService` 中增加关键词检索逻辑，使用 MyBatis-Plus `like` 条件查询切片正文，并按文档 ID、切片序号升序排列。
- 在 `KnowledgeController` 中增加 `GET /api/kb/search` 接口，通过 Query 参数 `keyword` 接收搜索关键词。
- 对关键词执行首尾空格清理，并对空关键词返回业务错误。
- 使用 Apifox 验证关键词正常匹配、无匹配结果、空关键词和缺少关键词参数等场景。
- 将搜索结果主键字段命名为 `chunkId`，与 `documentId` 明确区分。
- 运行 Maven 测试，确认后端项目编译和原有测试正常通过。

### 下一步

- 接入 DeepSeek 问答接口，将关键词检索结果作为知识库上下文生成回答。
