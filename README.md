...
- 已完成文档切片关键词检索接口 `GET /api/kb/search`，支持根据 `keyword` 查询包含关键词的切片。
- 搜索结果通过 `KbDocumentChunkSearchResultVO` 返回 `chunkId`、`documentId`、`chunkIndex` 和 `content`，并按文档 ID、切片序号升序排列。
- 已使用 Apifox 验证正常匹配、无匹配结果、空关键词和缺少关键词参数等场景。
...
```text
GET http://localhost:8080/api/kb/search?keyword=验证码
```

请求使用 Query 参数 `keyword`。匹配成功时返回切片 ID、所属文档 ID、切片序号和切片正文；没有匹配内容时返回空列表。
...
下一步是接入 DeepSeek 问答接口，将关键词检索出的文档切片作为知识库上下文生成回答。
...
`kb_document_chunk` 用于保存文档切片，通过 `document_id` 关联原文档，使用 `chunk_index` 记录切片顺序，并使用 `char_count` 记录每条切片的字符数。当前上传文档后会按固定 500 字自动切片，并支持使用关键词模糊查询切片正文。
